...
def process_ocr(FLAGS, images, detection_results):
    """
    Process OCR for detected bboxes
    """
    # Initialize PaddleOCR
    ocr = PaddleOCR(lang=FLAGS.ocr_lang, use_angle_cls=True)
    
    # Load label list for category mapping
    if os.path.exists(FLAGS.label_list_path):
        ...
        logger.warning(f"Label list file {FLAGS.label_list_path} not found, use class ID instead.")
        label_list = []
    ...
    for img_path, res in zip(images, detection_results):
        ...
        if 'bbox' in res and len(res['bbox']) > 0:
            for bbox_idx, bbox_info in enumerate(res['bbox']):
                # bbox_info format: [cls_id, score, x1, y1, x2, y2]
                cls_id, score, x1, y1, x2, y2 = bbox_info
                # Skip low confidence bboxes
                if score < FLAGS.draw_threshold:
                    continue
                
                # Get category name
                cls_name = label_list[int(cls_id)] if (label_list and int(cls_id) < len(label_list)) else f"class_{int(cls_id)}"
                
                # Crop image
                crop_path = os.path.join(FLAGS.crop_dir, f"{img_name}_{bbox_idx}.png")
                ...
                # Log result
                logger.info(f"\n===== Bbox {bbox_idx+1} of {img_name} =====")
                logger.info(f"Category: {cls_name} (ID: {int(cls_id)}), Confidence: {score:.2f}")
                logger.info(f"Bbox Coords: x1={x1:.1f}, y1={y1:.1f}, x2={x2:.1f}, y2={y2:.1f}")
...
def run(FLAGS, cfg):
    ...
    # inference
    detection_results = None  # 新增：保存检测结果
