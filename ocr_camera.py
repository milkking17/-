...
def run_realtime_detection(FLAGS, cfg):
    ...
    if not cap.isOpened():
        logger.error(f"无法打开摄像头（ID={CAMERA_ID}），请检查摄像头连接！")
        return
    ...
    try:
        while True:
            ...
            # 检测推理（将BGR帧转为RGB，适配PaddleDetection）
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            # 构造单帧检测输入
            single_image_input = [np.ascontiguousarray(frame_rgb)]
            # 执行检测（关闭可视化和保存，提升速度）
        ...
        logger.info("摄像头资源已释放，检测结束")

def main():
    FLAGS = parse_args()
    cfg = load_config(FLAGS.config)
    merge_args(cfg, FLAGS)  # 合并命令行参数（包括-o传递的weights）
